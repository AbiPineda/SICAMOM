...
CREATE  TABLE IF NOT EXISTS `Clinica`.`t_paciente` (
  ...
  `pac_ffecja_nac` DATE NULL ,
  `pac_ctipo_consulta` VARCHAR(25) NULL ,
  PRIMARY KEY (`id_paciente`) ,
  INDEX `fk_t_paciente_t_responsable1_idx` (`fk_responsable` ASC) ,
  CONSTRAINT `fk_t_paciente_t_responsable1`
    FOREIGN KEY (`fk_responsable` )
    REFERENCES `Clinica`.`t_responsable` (`idresponsable` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE  TABLE IF NOT EXISTS `Clinica`.`t_enfermeria_fetal` (
  `id_enfermeria_fetal` INT NOT NULL ,
...
CREATE  TABLE IF NOT EXISTS `Clinica`.`t_usuario` (
  ...
  `usu_ccontraseña` VARCHAR(10) NULL ,
